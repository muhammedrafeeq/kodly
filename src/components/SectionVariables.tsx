...
const NAMING_CHALLENGES: NamingChallenge[] = [
  {
    ...
    explanation: "C Naming Rule Violation: Variable names cannot start with a number. They must start with a letter.",
    options: [
      { label: "Valid C Name", value: "valid" },
      { label: "Violates C Rules", value: "violation" },
      { label: "Bad Convention (poor style)", value: "bad_convention" }
    ]
  },
  {
    name: "student Age",
    expectedCategory: "violation",
    explanation: "C Naming Rule Violation: Variable names cannot contain spaces. Use 'studentAge' or 'student_age' instead.",
    options: [
      { label: "Valid C Name", value: "valid" },
      { label: "Violates C Rules", value: "violation" },
      { label: "Bad Convention (poor style)", value: "bad_convention" }
  ...
  },
  {
    name: "return",
    expectedCategory: "violation",
    explanation: "C Naming Rule Violation: 'return' is a reserved keyword (a command word C already uses). You cannot use it as a custom variable name.",
    options: [
      { label: "Valid C Name", value: "valid" },
      { label: "Violates C Rules", value: "violation" },
      { label: "Bad Convention (poor style)", value: "bad_convention" }
  ...
  },
  {
    ...
    explanation: "Valid C Name! Uses 'camelCase' (capitalizing words after the first to resemble a camel's hump), making it clear and easy to read.",
    options: [
      { label: "Valid C Name", value: "valid" },
      { label: "Violates C Rules", value: "violation" },
      { label: "Bad Convention (poor style)", value: "bad_convention" }
  ...
  },
  {
    ...
    explanation: "Valid C syntax, but Bad Naming Convention: Single letters like 'x' do not explain what is stored inside. Use meaningful names like 'score' instead.",
    options: [
      { label: "Valid C Name", value: "valid" },
      { label: "Violates C Rules", value: "violation" },
      { label: "Bad Convention (poor style)", value: "bad_convention" }
...
export default function SectionVariables({ onComplete }: SectionVariablesProps) {
  // Sub-step inside Variables: 0 = Concept/Locker, 1 = Naming Game, 2 = Reading & Formulas
  const [subStep, setSubStep] = useState<number>(0);

  // Sub-step 1: Locker States ('declare' | 'init' | 'assign')
        ...
        <div className="space-y-6 animate-fadeIn">
          ...
          <section className="glass-panel p-4 rounded-xl space-y-2 text-left">
            ...
            <p className="text-body-md text-on-surface-variant leading-relaxed font-sans">
              Think of a <span className="text-primary font-bold">variable</span> as a labeled **locker** inside your computer&apos;s memory. 
              It is a reserved space where your program can store, read, and change values. 
              Each locker has a **name tag** on the door so we can find it easily, and a **type** that specifies what kind of contents fit inside.
            </p>
          </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-stretch">
            ...
            <div className="md:col-span-5 glass-panel p-4 rounded-xl flex flex-col items-center justify-center min-h-[300px] relative overflow-hidden">
              ...
              <span className="text-[9px] font-mono text-on-surface-variant mt-3 text-center leading-relaxed block px-2">
                Memory Address: 0x7FFE <br />
                <span className="text-[8px] opacity-75">(The computer&apos;s physical location code, which we name &quot;age&quot; so we don&apos;t have to memorize it!)</span>
              </span>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-surface-container-lowest p-4 rounded-xl border border-white/5 font-mono text-xs text-left space-y-2 relative overflow-hidden shadow-inner">
                  ...
                  <div className={`p-2 rounded transition-all duration-300 flex items-center justify-between cursor-pointer ${
                    ...
                    <span>1. int age;</span>
                    <span className="text-[9px] font-sans font-bold uppercase tracking-wider">{lockerState === "declare" ? "◀ DECLARE" : ""}</span>
                  </div>
                  ...
                  <div className={`p-2 rounded transition-all duration-300 flex items-center justify-between cursor-pointer ${
                    ...
                    <span>2. age = 25;</span>
                    <span className="text-[9px] font-sans font-bold uppercase tracking-wider">{lockerState === "init" ? "◀ INITIALIZE" : ""}</span>
                  </div>
                  ...
                  <div className={`p-2 rounded transition-all duration-300 flex items-center justify-between cursor-pointer ${
                    ...
                    <span>3. age = 30;</span>
                    <span className="text-[9px] font-sans font-bold uppercase tracking-wider">{lockerState === "assign" ? "◀ REASSIGN" : ""}</span>
                  </div>
                </div>
                  ...
                  {lockerState === "declare" && (
                    <div className="animate-fadeIn space-y-2">
                      <div className="font-bold text-error flex items-center gap-1">
                        ...
                        Declaration: <code className="text-on-surface font-normal bg-white/5 px-1 py-0.5 rounded">int age;</code>
                      </div>
                      <p className="text-xs text-on-surface-variant/95 leading-relaxed font-sans">
                        This creates the storage locker in memory, but puts **no value** inside yet.
                        <br />
                        <span className="text-error font-bold block mt-1.5">⚠️ THE GARBAGE GOTCHA:</span>
                        In C, if you don&apos;t put a value in, the locker will contain random leftovers from whatever program used that memory slot before (a **Garbage Value**). Reading this will print random nonsense numbers!
                      </p>
                    </div>
                  )}
                  ...
                  {lockerState === "init" && (
                    <div className="animate-fadeIn space-y-2">
                      <div className="font-bold text-primary flex items-center gap-1">
                        ...
                        Initialization: <code className="text-on-surface font-normal bg-white/5 px-1 py-0.5 rounded">int age = 25;</code>
                      </div>
                      <p className="text-xs text-on-surface-variant/95 leading-relaxed font-sans">
                        This creates the locker **AND** fills it with a starting value (`25`) at the exact same time. This is the safest way to declare variables and avoids the garbage value gotcha.
                      </p>
                    </div>
                  )}
                  ...
                  {lockerState === "assign" && (
                    <div className="animate-fadeIn space-y-2">
                      <div className="font-bold text-secondary flex items-center gap-1">
                        ...
                        Assignment: <code className="text-on-surface font-normal bg-white/5 px-1 py-0.5 rounded">age = 30;</code>
                      </div>
                      <p className="text-xs text-on-surface-variant/95 leading-relaxed font-sans">
                        This changes the value inside an existing locker. The old value (`25`) is discarded, and the new value (`30`) takes its place. Notice there is no <code className="text-secondary font-bold">int</code> keyword here because the locker already exists!
                      </p>
                    </div>
                  )}
        ...
        <div className="space-y-6 animate-fadeIn text-left">
          ...
          <section className="glass-panel p-4 rounded-xl space-y-2">
            <h3 className="text-sm font-bold text-secondary flex items-center gap-1.5">
              ...
              C Naming Rules &amp; Conventions
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs font-mono text-on-surface-variant pt-1">
              <div className="space-y-1">
                <span className="text-error font-bold block">❌ STRICT RULES (C Compiler):</span>
                • Cannot start with numbers (e.g. `1st`)
                <br />• Cannot contain spaces inside
                <br />• Cannot use reserved commands (e.g. `int`)
                <br />• <span className="text-on-surface underline">Case-sensitive:</span> Capitalization matters! `age` and `Age` are completely different variables.
              </div>
              <div className="space-y-1">
                <span className="text-primary-fixed-dim font-bold block">💡 BEST PRACTICES (Conventions):</span>
                • <span className="text-on-surface underline">camelCase:</span> Capitalize words after the first (e.g. `studentAge`) so it reads like camel humps.
                <br />• Use meaningful names instead of single letters (e.g. `score` instead of `x` or `a1`).
              </div>
            </div>
          </section>
          ...
          <section className="glass-panel p-5 rounded-xl space-y-4 border-secondary/20">
            ...
            {answerSubmitted && (
              <div className="flex justify-end pt-2">
                ...
                >
                  {challengeIdx < NAMING_CHALLENGES.length - 1 ? "NEXT CHALLENGE" : "GO TO LIVE EXPRESSIONS"}
                </button>
              </div>
            )}
          </section>
        ...
        <div className="space-y-6 animate-fadeIn text-left">
          ...
          <section className="glass-panel p-4 rounded-xl space-y-2">
            ...
            <p className="text-body-md text-on-surface-variant leading-relaxed">
              Once you store a value in a locker, your program can read it at any time in two ways:
            </p>
            <ul className="list-disc list-inside text-xs text-on-surface-variant space-y-1.5 pl-2 font-sans">
              <li>
                <span className="font-bold text-on-surface">Printing:</span> Displays it on the screen using placeholder codes (like <code className="text-primary-fixed-dim bg-white/5 px-1 py-0.5 rounded">%d</code>).
              </li>
              <li>
                <span className="font-bold text-on-surface">Math Calculations:</span> Using the variable&apos;s name in formulas (e.g. <code className="text-primary-fixed-dim bg-white/5 px-1 py-0.5 rounded">age + 5</code>) to calculate new values.
              </li>
            </ul>
          </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-stretch">
            ...
            <div className="glass-panel rounded-xl overflow-hidden flex flex-col justify-between border-tertiary/20">
              <div className="bg-surface-container-high px-3 py-1.5 border-b border-white/5 text-[10px] font-mono text-on-surface-variant flex items-center gap-1.5">
                ...
                <span>expressions_sandbox.c</span>
              </div>
            ...
            <div className="glass-panel rounded-xl overflow-hidden flex flex-col justify-between border border-white/5">
              ...
              <div className="p-4 bg-surface-container-lowest font-code-md text-xs text-primary-fixed-dim flex-1 flex flex-col justify-center gap-1.5">
                <div>&gt; gcc expressions_sandbox.c -o test</div>
                <div>&gt; ./test</div>
          ...
          <div className="p-3 bg-secondary/10 border border-secondary/20 rounded-xl text-xs leading-relaxed flex items-center gap-2.5">
            ...
            <p className="text-on-surface-variant/95">
              <strong className="text-secondary">Bridge to Section 2:</strong> In C, every variable must have a datatype (like <code className="text-primary-fixed-dim">int</code>) so the computer knows what shape of box to build. Let&apos;s go to the next section to explore datatypes!
            </p>
          </div>
